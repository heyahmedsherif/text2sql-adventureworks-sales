...
class AISearch(ABC):
    ...
    def _ensure_storage_container_exists(self):
        """Ensure the required storage container exists before creating the data source."""
        try:
            blob_service_client = BlobServiceClient.from_connection_string(
                self.environment.storage_account_connection_string
            )
            container_name = self.environment.storage_account_blob_container_name
            container_client = blob_service_client.get_container_client(container_name)
            
            if not container_client.exists():
                container_client.create_container()
                logging.info("Created storage container: %s", container_name)
            else:
                logging.info("Storage container already exists: %s", container_name)
                
        except Exception as e:
            logging.error("Failed to create storage container: %s", e)
            raise
    ...
    def deploy_data_source(self):
        ...
        # Ensure the storage container exists before creating the data source
        self._ensure_storage_container_exists()
