...
def deploy_config_fixed(arguments: argparse.Namespace):
    """Deploy the indexer configuration based on the arguments passed.
    Fixed version that calls deploy methods individually to avoid the deploy() bug.
    ...
    # Fixed deploy: Call methods individually instead of deploy()
    print(f"=== Deploying {arguments.index_type} ===")
    
    try:
        print("Step 1: Ensuring storage container exists...")
        index_config._ensure_storage_container_exists()
        
        print("Step 2: Deploying data source...")
        index_config.deploy_data_source()
        
        print("Step 3: Deploying synonym map...")
        index_config.deploy_synonym_map()
        
        print("Step 4: Deploying index...")
        index_config.deploy_index()
        
        print("Step 5: Deploying skillset...")
        index_config.deploy_skillset()
        
        print("Step 6: Deploying indexer...")
        index_config.deploy_indexer()
